      ...
      <section id="sponsors-section" className="py-20 bg-white">
        <div className="container mx-auto px-4 sm:px-6 lg:px-8">
          <motion.div
            id="sponsors-title"
            initial={{ opacity: 0, y: 30 }}
            animate={sponsorsTitleVisible ? { opacity: 1, y: 0 } : { opacity: 0, y: 30 }}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {currentSponsors.map((sponsor, index) => (
              <motion.div
                ...
                animate={sponsorsVisible ? { opacity: 1, y: 0 } : { opacity: 0, y: 50 }}
      ...
      <section id="tiers-section" className="py-20 bg-osu-gray-light">
        <div className="container mx-auto px-4 sm:px-6 lg:px-8">
          <motion.div
            id="tiers-title"
            initial={{ opacity: 0, y: 30 }}
            animate={tiersTitleVisible ? { opacity: 1, y: 0 } : { opacity: 0, y: 30 }}
            whileInView={{ opacity: 1, y: 0 }}
            transition={{ duration: 0.8 }}
            viewport={{ once: true, margin: '-200px', amount: 0.1 }}
            onViewportEnter={() => {
      ...
      <section id="partners-section" className="py-20 bg-white">
        <div className="container mx-auto px-4 sm:px-6 lg:px-8">
          <motion.div
            id="partners-title"
            initial={{ opacity: 0, y: 30 }}
            animate={partnersTitleVisible ? { opacity: 1, y: 0 } : { opacity: 0, y: 30 }}
            whileInView={{ opacity: 1, y: 0 }}
            transition={{ duration: 0.8 }}
            viewport={{ once: true, margin: '-200px', amount: 0.1 }}
            onViewportEnter={() => {
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {partnerOrganizations.map((partner, index) => (
              <motion.div
                key={index}
                initial={{ opacity: 0, y: 30 }}
                animate={partnersVisible ? { opacity: 1, y: 0 } : { opacity: 0, y: 30 }}
                transition={{ duration: 0.6, delay: index * 0.05 }}
